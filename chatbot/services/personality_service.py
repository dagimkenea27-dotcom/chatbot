...
class PersonalityService:
    ...
    def apply_enhancement(self, response: str, session, intent: str, sentiment: str) -> str:
        ...
        if "━━━" in response or "PROMO" in response or "CHECKOUT" in response or response.startswith("⚠️") or response.startswith("["):
            return response
    ...
    def wrap_natural(self, response: str, session, intent: str) -> str:
        ...
        if intent in ("farewell", "human_support", "small_talk", "greeting", "checkout") or "━━━" in response or "PROMO" in response or "CHECKOUT" in response or response.startswith("⚠️") or response.startswith("["):
            return response
